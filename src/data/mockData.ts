...
// Этот файл — временный data-layer для прототипа.
// Он имитирует backend и позволяет развивать UI/UX независимо от API.
...
export function getEventsByTopicId(topicId: string): Event[] {
  // Явная маршрутизация topicId -> коллекция событий.
  // При реальном backend это заменится на API-запрос.
  switch (topicId) {
...
export function getEventById(eventId: string): Event | undefined {
  // Собираем единый плоский массив, чтобы имитировать поиск по БД.
  const allEvents = [
...
export function getTopicById(topicId: string): Topic | undefined {
  // Поиск темы в текущем наборе топиков.
