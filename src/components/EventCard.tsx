...
export default function EventCard({ event, onPress }: EventCardProps) {
  ...
  // Короткая карточка события внутри выбранной темы.
  // Нужна для быстрого сканирования заголовка, summary и фактов.
  return (
    <TouchableOpacity
      style={styles.card}
      onPress={onPress}
    ...
    >
      ...
      {/* Ключевые факты (первые 2) */}
      <View style={styles.facts}>
        {event.keyFacts.slice(0, 2).map((fact, index) => (
          <View key={index} style={styles.factItem}>
            <View style={styles.factDot} />
            <Text style={styles.factText} numberOfLines={1}>{fact}</Text>
          </View>
        ))}
      </View>
