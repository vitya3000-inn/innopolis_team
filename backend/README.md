...
- `GET /events/:eventId`
- `POST /admin/refresh` — ручной ingestion (нужен JWT админа, если задан список админов)
- `GET /admin/visit-stats?from=YYYY-MM-DD&to=YYYY-MM-DD` — число посещений ленты из таблицы `app_visits` (см. `supabase/app_visits.sql`)
